...
int recv_neighbor(){
    ...
    j=0;
    MPI_Request req2[ctx.number_of_neighbors];
    for(int i = 0;i<ctx.number_of_neighbors;i++){
        int rank = ctx.neighbor_ranks[i];
...
int get_slice_from_position(struct fish* f){
    float slice_size = (float) params.edge_size/ctx.world_size;
...
void play(){
    ...
    printf("%d- starting the simulation\n",ctx.my_rank);
...
void setup(){
    ...
    ctx.end_y = (float)((float) params.edge_size/ctx.world_size) * (ctx.my_rank+1);
    ctx.start_y = (float)((float) params.edge_size/ctx.world_size) * (ctx.my_rank);
    ctx.adjacent_to_consider = ceil(params.eating_distance/(ctx.end_y - ctx.start_y));
    printf("%d-adjacent to consider %ld\n",ctx.my_rank,ctx.adjacent_to_consider);
